...
function localDraftFromPrompt(prompt) {
  const p = String(prompt || '').trim();
  const lower = p.toLowerCase();
  const topic = p.replace(/^(tao|tạo|viet|viết)\s+/i, '').trim() || 'Tin hoc van phong';

  if (/zalo|bài đăng|bai dang|facebook|fanpage/.test(lower)) {
    return [
      `📢 ${topic.toUpperCase()}`,
      '',
      '🎯 Khoa hoc Word - Excel - PowerPoint chuan quoc te',
      '✅ Hoc thuc hanh 100% - Gia vien 1 ke 1',
      '✅ Cam ket dau chung chi MOS',
      '',
      '📞 Hotline: 097xxxxxxx',
      '🌐 Website: thangtinhoc.vn',
      '',
      '#ThangTinHoc #TinHocVanPhong #MOS #Word #Excel #PowerPoint',
    ].join('\n');
  }

  if (/thong bao|thông báo|nhac|nhắc|hoc phi|học phí/.test(lower)) {
    return [
      'Kinh gui quy phu huynh va hoc vien,',
      '',
      `Trung tam Thang Tin Hoc xin thong bao: ${topic}.`,
      'Mong quy vi sap xep thoi gian tham gia day du.',
      '',
      'Tran trong,',
      'Phong Dao tao - Thang Tin Hoc',
    ].join('\n');
  }

  return [
    `Noi dung goi y cho: ${topic}`,
    '',
    '- Mo dau: gioi thieu ngan gon muc tieu',
    '- Than bai: 3 diem noi bat (thuc hanh, giao vien, chung chi)',
    '- Ket: loi keu goi hanh dong (dang ky / lien he)',
    '',
    '(Ban nhap local — ket noi Gemini de sinh noi dung chi tiet hon)',
  ].join('\n');
}

function localNotification(purpose, audience) {
  return {
    title: 'Thong bao: ' + String(purpose || '').slice(0, 80),
    content: [
      `Kinh gui ${audience || 'hoc vien'},`,
      '',
      String(purpose || ''),
      '',
      'Tran trong,',
...
function humanizeAiError(raw) {
  ...
  if (/401|UNAUTHENTICATED|ACCESS_TOKEN_TYPE_UNSUPPORTED|invalid authentication/i.test(`${inner} ${msg}`)) {
    return 'Google chua chap nhan key. Vao Cloud Console bat "Generative Language API", roi AI Studio chon "Restrict to Gemini API only".';
  }
...
function getStatus() {
  ...
  const cfg = getConfig();
  const key = process.env.GEMINI_API_KEY || process.env.AI_API_KEY || process.env.OPENAI_API_KEY || '';
  const provider = process.env.GEMINI_API_KEY ? 'gemini' : (process.env.AI_API_KEY || process.env.OPENAI_API_KEY ? 'openai' : null);
  return {
    configured,
    ...
    provider,
    keyType: key.startsWith('AQ.') ? 'auth' : key.startsWith('AIza') ? 'standard' : key ? 'other' : null,
...
async function probeHealth() {
  ...
  if (!base.configured) {
    return { ...base, healthy: false, healthError: 'Chua co API key trong .env (GEMINI_API_KEY hoac AI_API_KEY)' };
  }
  try {
    await chatCompletion({ messages: [{ role: 'user', content: 'ping' }], maxTokens: 8 });
    return { ...base, healthy: true, healthError: null };
  } catch (e) {
    return { ...base, healthy: false, healthError: humanizeAiError(e.message) };
  }
}
...
async function generateQuiz({ topic, count = 5, subject = 'Tin hoc van phong' } = {}) {
  ...
  try {
    ...
    logger.warn({ err: humanizeAiError(e.message) }, '[AI] quiz local fallback');
    return fallbackQuiz({ topic: t, count: n, subject });
  }
}
...
async function draftNotification({ purpose, audience = 'all', tone = 'professional' } = {}) {
  ...
  if (!isAiConfigured()) {
    return {
      title: localNotification(p, audience).title,
      content: localNotification(p, audience).content,
      source: 'local',
      model: null,
    };
  }
  try {
    ...
    logger.warn({ err: e.message }, '[AI] notification local fallback');
    const local = localNotification(p, audience);
    return { ...local, source: 'local', model: null };
  }
}
...
async function complete({ prompt, system } = {}) {
  ...
  if (!isAiConfigured()) {
    return {
      content: localDraftFromPrompt(p),
      source: 'local',
      model: null,
    };
  }
  ...
  try {
    ...
    logger.warn({ err: e.message }, '[AI] complete local fallback');
    return {
      content: localDraftFromPrompt(p),
      source: 'local',
      model: null,
