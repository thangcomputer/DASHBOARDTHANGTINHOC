...
const { backupJobRepository } = require('../../report/repositories');
const BackupJob = require('../../report/models/BackupJob');
...
async function runBackupJob(jobId) {
  const job = await backupJobRepository.findById(jobId);
...
async function createBackupJob({ type = 'manual', createdBy = '' } = {}) {
  return backupJobRepository.create({
    status: 'pending',
    type,
...
async function listBackups({ page = 1, limit = 20 } = {}) {
  ...
  const [rows, total] = await Promise.all([
    backupJobRepository.findMany().sort({ createdAt: -1 }).skip(skip).limit(limitNum).lean(),
    backupJobRepository.count(),
  ]);
  return {
    data: rows.map((r) => ({
      ...r,
...
async function getBackupFile(jobId) {
  const job = await backupJobRepository.findById(jobId).lean();
...
async function enforceRetention() {
  ...
  const completed = await backupJobRepository.findMany({ status: 'completed' }).sort({ createdAt: -1 }).lean();
...
async function getStats() {
  const [total, completed, failed, last] = await Promise.all([
    backupJobRepository.count(),
    backupJobRepository.count({ status: 'completed' }),
    backupJobRepository.count({ status: 'failed' }),
    backupJobRepository.findOne({ status: 'completed' }).sort({ createdAt: -1 }).lean(),
  ]);
  let diskBytes = 0;
  ensureBackupDir();
