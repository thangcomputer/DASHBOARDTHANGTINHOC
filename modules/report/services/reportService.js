 ...
 */
const { reportDefinitionRepository } = require('../../report/repositories');
const ReportDefinition = require('../../report/models/ReportDefinition');
const FormDefinition = require('../../cms/models/FormDefinition');
const FormSubmission = require('../../cms/models/FormSubmission');
...
async function createReport({ name, description, source, columns, filters, createdBy }) {
  ...
  return reportDefinitionRepository.create({
...
async function updateReport(id, patch) {
  const report = await reportDefinitionRepository.findById(id);
  if (!report) {
...
async function listReports({ page = 1, limit = 30 } = {}) {
  ...
  const [data, total] = await Promise.all([
    reportDefinitionRepository.findMany().sort({ updatedAt: -1 }).skip(skip).limit(limitNum).lean(),
...
async function deleteReport(id) {
  const r = await reportDefinitionRepository.deleteById(id);
  if (!r) {
    const err = new Error('Khong tim thay bao cao');
    err.status = 404;
    throw err;
...
async function runReport(id, { limit = 500 } = {}) {
  const report = await reportDefinitionRepository.findById(id).lean();
  if (!report) {
    const err = new Error('Khong tim thay bao cao');
    err.status = 404;
    throw err;
