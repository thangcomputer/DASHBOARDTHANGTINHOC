'use strict';
const systemLogApplicationService = require('../services/SystemLogApplicationService');

const { SystemLogValidator, SystemLogMapper } = require('../dto');

class SystemLogController {
  async get_root(req, res) {
    try {
      // Replaced by Zod Validator
      const result = await systemLogApplicationService.get_root(SystemLogValidator.validateGet_root(req));
      if (!result) return;
  ...
  async post_root(req, res) {
    try {
      // Replaced by Zod Validator
      const result = await systemLogApplicationService.post_root(data);
      if (!result) return;
      if (result._isSend) return res.status(result._status ?? 200).send(result._body);
      return res.status(result._status ?? 200).json(result._body);
    } catch (err) {
  ...
  async delete_id(req, res) {
    try {
      // Replaced by Zod Validator
      const result = await systemLogApplicationService.delete_id(SystemLogValidator.validateDelete_id(req));
      if (!result) return;
      if (result._isSend) return res.status(result._status ?? 200).send(result._body);
      return res.status(result._status ?? 200).json(result._body);
    } catch (err) {
