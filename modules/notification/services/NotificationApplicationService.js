...
async function listForUser(user, { page = 1, limit = 20, type, unreadOnly = false } = {}) {
  ...
  const [rows, total, unread] = await Promise.all([
    notificationRepository.findPaginated(filter, skip, limitNum),
    notificationRepository.count(filter),
    notificationRepository.count(unreadFilter),
  ]);
...
async function countUnread(user) {
  ...
  const count = await notificationRepository.count(filter);
  return count;
}
...
async function markRead(user, { notificationId, markAll } = {}) {
  const { userId, match } = buildReceiverMatch(user);
  if (markAll) {
    const filter = { $or: match, dismissed_by: { $ne: userId }, read_by: { $ne: userId } };
    await notificationRepository.markAllAsRead(filter, userId);
    return { marked: 'all' };
  }
  if (!notificationId) {
...
async function dismiss(user, notificationId) {
  ...
  const doc = await notificationRepository.markAsDismissed(notificationId, userId);
  if (!doc) {
