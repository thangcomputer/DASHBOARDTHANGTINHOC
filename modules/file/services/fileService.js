...
const { fixMojibakeText, normalizeMulterFile } = require('../../../utils/escapeRegex');
...
async function deleteAsset(asset, { hard = true, finalStatus = 'deleted' } = {}) {
  ...
  await FileRepository.save(asset);
  return asset;
}

async function deleteById(id, user) {
  const asset = await FileRepository.findById(id);
...
async function listAssets({
  ...
  const [rows, total] = await Promise.all([
    FileRepository.findActiveAssets(filter, skip, limitNum),
    FileRepository.countActiveAssets(filter),
  ]);
  return {
    data: rows.map((row) => ({
      ...row,
      originalName: fixMojibakeText(row.originalName || row.filename),
    })),
    pagination: {
      page: pageNum,
      limit: limitNum,
...
async function getStats() {
  const byCategory = await FileRepository.aggregateStats();
...
async function purgeExpired() {
  const now = new Date();
  const expired = await FileRepository.findExpired(200);
  let purged = 0;
