...
class BiValidator {
  ...
  static validateGet_overview(req) {
    ...
    if (!result.success) {
      ValidationMetrics.logFailure('finance', 'Get_overviewQuery', Date.now() - start, result.error.errors);
    ...
    ValidationMetrics.logSuccess('finance', 'Get_overviewQuery', Date.now() - start);
    return Object.freeze(result.data);
  }

  static validateGet_export(req) {
    const schema = z.object({
    branchId: z.any().optional(),
    period: z.any().optional(),
    }).passthrough();
    ...
    if (!result.success) {
      ValidationMetrics.logFailure('finance', 'Get_exportQuery', Date.now() - start, result.error.errors);
      const errors = result.error.errors.map(err => ({ field: err.path.join('.'), code: 'invalid_field', message: err.message }));
      throw new ValidationException('Invalid request', errors);
    }
    
    ValidationMetrics.logSuccess('finance', 'Get_exportQuery', Date.now() - start);
    return Object.freeze(result.data);
  }
}

module.exports = BiValidator;
