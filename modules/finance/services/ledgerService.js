...
const LedgerEntry = require('../models/LedgerEntry'); // Temp for new LedgerEntry
...
async function postEntry(payload) {
  ...
  try {
    ...
    if (err && err.code === 11000) {
      const existing = await ledgerRepository.findOne({ idempotencyKey: key });
...
async function sumFinancialRevenue({
  ...
  const rows = await ledgerRepository.aggregateTotalsByType([
    { $match: match },
    {
      $group: {
        _id: '$type',
...
async function listLedgerEntries({
  ...
  const [items, total] = await Promise.all([
    ledgerRepository.findMany(match)
      .sort({ postedAt: -1, createdAt: -1 })
      .skip(skip)
      .limit(lim)
      .lean(),
    ledgerRepository.count(match),
  ]);
...
async function healOrphanEnrollmentPayments(student) {
  ...
  const payments = await ledgerRepository.findMany({
...
async function issueCreditNoteForRefund({
  ...
  const { creditNoteRepository } = require('../repositories');
const CreditNote = require('../models/CreditNote'); // Temp for new CreditNote
  const amt = Math.abs(Number(amount) || 0);
  if (!(amt > 0)) return null;

  const d = new Date();
  const ym = `${String(d.getFullYear()).slice(-2)}${String(d.getMonth() + 1).padStart(2, '0')}`;
  const n = await creditNoteRepository.count();
  const maChungTu = `CN${ym}-${String(n + 1).padStart(4, '0')}`;

  try {
    return await creditNoteRepository.create({
      maChungTu,
      studentId: student?._id || null,
      branchId: student?.branchId || null,
...
async function rebuildDailySnapshots({ from = null, to = null, branchId = null } = {}) {
  const { financeDailySnapshotRepository } = require('../repositories');
const FinanceDailySnapshot = require('../models/FinanceDailySnapshot'); // Temp for new FinanceDailySnapshot
  const match = { status: 'posted' };
  if (branchId) match.branchId = toObjectId(branchId);
  if (from || to) {
    match.postedAt = {};
  ...
  for (const slot of map.values()) {
    ...
    await financeDailySnapshotRepository.updateOne(
...
async function revenueByCourseFromLedger({
  ...
  return ledgerRepository.aggregateTotalsByType([
    { $match: match },
    {
      $group: {
