...
function extractDomain({ domain, routeFile, serviceName, controllerName }) {
  ...
  while (true) {
    ...
    // consume final semicolon and newline
  ...
  // Rewrite Route file – replace each async handler with controller delegation
...
// Teacher routes
for (const [routeFile, svc, ctrl] of [
  ['modules/teacher/routes/teacherRoutes.js', 'TeacherApplicationService', 'TeacherController'],
  ['modules/teacher/routes/employeeRoutes.js', 'EmployeeApplicationService', 'EmployeeController'],
  ['modules/teacher/routes/staffRoutes.js', 'StaffApplicationService', 'StaffController'],
]) {
  try {
    extractDomain({ domain: `teacher/${routeFile.split('/').pop()}`, routeFile: path.join(BASE, routeFile), serviceName: svc, controllerName: ctrl });
  } catch (e) { console.error(`${routeFile} ERROR:`, e.message); }
}
...
try {
  extractDomain({
    domain: 'student',
    routeFile: path.join(BASE, 'modules/student/routes/studentRoutes.js'),
    serviceName: 'StudentApplicationService',
