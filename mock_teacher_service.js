const fs = require('fs');
const content = fs.readFileSync('d:\\web\\WEB TỔNG HỢP\\DASHBOARDTHANGTINHOC\\modules\\teacher\\services\\TeacherApplicationService.js', 'utf8');
...
let mockFile = `'use strict';
const logger = require('../../../config/logger');

class TeacherApplicationService {
`;

// Inject the exact working post_root
mockFile += `  async post_root(data) {
  try {
...
methods.forEach(m => {
  mockFile += `  async ${m}() { return { _status: 501, _body: { success: false, message: 'Not implemented' } }; }\n`;
});
...
fs.writeFileSync('d:\\web\\WEB TỔNG HỢP\\DASHBOARDTHANGTINHOC\\modules\\teacher\\services\\TeacherApplicationService.js', mockFile, 'utf8');
console.log('Mocked TeacherApplicationService.js successfully.');
