...
async function upload() {
  ...
  console.log('\n=== 2. UPLOAD NEW DIST ===');
  const s1 = await ssh.putDirectory(localDist, '/www/wwwroot/dashboardthangtinhoc/client/dist', { recursive: true, concurrency: 5 });
  ...
  const updateEnv = await ssh.execCommand("sed -i 's|CLIENT_URL=http://dashboard|CLIENT_URL=https://dashboard|' /www/wwwroot/dashboardthangtinhoc/.env");
