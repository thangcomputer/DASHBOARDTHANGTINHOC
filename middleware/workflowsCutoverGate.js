/**
 * Phase 7.9 — Controlled cutover gate for /api/workflows ONLY.
 *
 ...
 * Does not advance workflows, sync, write DB, emit sockets, enqueue jobs, or mutate auth.
 * Advance / realtime / DB mutations remain in Legacy handlers + workflowService.
 */
...
function denyWorkflow(res, statusHint) {
...
/**
 * @param {string} action - workflowPolicy action key
 */
function workflowsCutoverGate(action) {
  return (req, res, next) => {
    let authority = AUTHORITY.LEGACY;
    try {
      authority = getAuthorizationAuthority('workflows');
    } catch (err) {
      ...
      logger.warn(
        ...
        '[POLICY_CUTOVER] workflows authority helper failed — Legacy gate',
      );
    }

    req.authzAuthority = authority;
    req.authzFamily = 'workflows';
    ...
    if (authority !== AUTHORITY.POLICY) {
      logger.info(
        ...
        '[POLICY_CUTOVER] workflows using Legacy authority',
      );
    ...
    if (
      ...
      logger.warn(
        ...
        '[POLICY_CUTOVER] workflows Policy unsafe — fallback Legacy isAdmin',
      );
    ...
    if (decision === 'ALLOW') {
      ...
      logger.info(
        ...
        '[POLICY_CUTOVER] workflows Policy ALLOW — Legacy isAdmin skipped',
      );
      return next();
    }
    ...
    logger.info(
      ...
      '[POLICY_CUTOVER] workflows Policy DENY',
    );
    return denyWorkflow(res, statusHint);
  };
}

module.exports = {
  workflowsCutoverGate,
  denyWorkflow,
};
