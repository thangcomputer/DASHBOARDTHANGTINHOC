/**
 * Policy SHADOW for /api/analytics. Always next(); never alters HTTP or aggregates.
 */
...
function policyShadowAnalytics(action) {
  return async (req, res, next) => {
    try {
      ...
      try {
        legacy = evaluateLegacyAnalytics(subject, action, ctx);
        policy = evaluatePolicyAnalytics(subject, action, ctx, untrusted);
        ...
        logger.warn(
          ...
          '[POLICY_SHADOW] analytics evaluation error — legacy still authoritative',
        );
        req.policyShadow = { action: `analytics_${action}`, comparison: 'ERROR', error: evalErr.message };
        return next();
      }
      ...
      if (comparison === 'MISMATCH') {
        logger.warn(
          ...
          '[POLICY_MISMATCH] analytics shadow disagrees — HTTP unchanged',
        );
      }
    } catch (err) {
      logger.warn(
        ...
        '[POLICY_SHADOW] unexpected analytics error — fail closed to legacy only',
      );
    }
    return next();
  };
...
module.exports = { policyShadowAnalytics };
