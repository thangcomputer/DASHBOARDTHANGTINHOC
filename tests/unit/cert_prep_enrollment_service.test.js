...
function installStubs(db) {
  ...
  CertPrepEnrollmentMapping.find = (q = {}) => ({
    lean: async () => db.mappings.filter((m) => (q.isActive == null ? true : m.isActive === q.isActive)),
    populate() { return this; },
    sort() { return this; },
  });
  CertPrepEnrollmentMapping.findOne = (q) => ({
    lean: async () => db.mappings.find((m) => String(m.courseId) === String(q.courseId)
      && (q.isActive == null || m.isActive === q.isActive)) || null,
  });
...
test('pending_payment enrollment is not qualifying', () => {
...
test('CASE 11: cancel enrollment does not revoke or delete CertPrep access', () => {
  ...
  assert.equal(block.includes('syncCertPrepFromEnrollment'), false);
  assert.equal(block.includes('StudentCertPrepAccess'), false);
  assert.equal(block.includes('CertPrepSession'), false);
});

test('CASE 15: enrollment bridge never deletes sessions or access', () => {
