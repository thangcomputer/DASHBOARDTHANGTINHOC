...
export function exportStudentQuestionsExcel(questions, subjectId, subjectLabel, questionType = 'both') {
  ...
  const filtered = (questions || []).filter((q) => {
    ...
    const isEssay = String(q.type).toLowerCase() === 'essay';
    if (questionType === 'essay') return isEssay;
    if (questionType === 'multiple') return !isEssay;
    return true;
  });
  const rows = filtered.map((q) => questionRowToExcel(q, label));
  if (!rows.length) rows.push(questionRowToExcel({ type: 'multiple', section: subjectId, q: '', options: ['', '', '', ''], correct: 0, difficulty: 'medium' }, label));
  const ws = XLSX.utils.json_to_sheet(rows, { header: STUDENT_QUESTIONS_TEMPLATE_HEADERS });
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Cau hoi');
  const suffix = questionType === 'essay' ? 'TuLuan' : questionType === 'multiple' ? 'TracNghiem' : 'DayDu';
  const safe = String(label).replace(/[^\w\u00C0-\u024F]+/g, '_').slice(0, 40);
  XLSX.writeFile(wb, `NganHang_${suffix}_${safe}.xlsx`);
}
