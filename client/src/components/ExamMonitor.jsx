...
const ExamMonitor = forwardRef(({ isActive, onViolate, requireWebcam = true }, ref) => {
  ...
  return (
    <>
      ...
      />
      {warningOverlay && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-red-950/90 backdrop-blur-md p-4 sm:p-6 animate-in fade-in duration-300">
      ...
      >
        ...
        {!stats.lastFaceDetected && (
          <div className="absolute bottom-0.5 left-0 right-0 text-center px-0.5">
            <span className="text-xs cms-min-text-xs leading-tight text-amber-200 font-black bg-black/70 px-1 py-0.5 rounded block">
              Căn mặt vào oval
            </span>
          </div>
        )}
