                ...
                <div className="bg-white rounded-[32px] p-6 md:p-8 border border-slate-100 shadow-xl flex flex-col md:flex-row gap-6 md:gap-8 items-stretch">
                  <div className="flex flex-col items-center justify-center text-center space-y-3 md:w-[35%] md:border-r border-slate-100 md:pr-8 py-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs cms-min-text-xs md:text-xs text-slate-400 font-black uppercase tracking-[0.2em] mt-1">Giảng viên trực tiếp</p>
                    </div>
                    {!hasRated || isEditing ? (
                      <div className="w-full pt-4 md:pt-6 space-y-2">
                        <p className="text-xs cms-min-text-xs font-black text-slate-500 uppercase tracking-widest text-left px-1">LỜI NHẮN CHO GIẢNG VIÊN (Tùy chọn):</p>
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    ...
                    <div className="bg-yellow-50/50 rounded-[32px] p-8 border border-yellow-100 space-y-6 text-center">
                       <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs font-black text-yellow-700/50 uppercase tracking-widest mt-2">Điểm bạn đã đánh giá</p>
                       </div>
                       ...
                       <button onClick={() => setIsEditingRating(true)} className="px-8 py-3 bg-white border-2 border-slate-100 rounded-2xl text-xs font-black text-slate-400 hover:text-slate-600 hover:border-yellow-200 uppercase tracking-widest flex items-center justify-center gap-2 mx-auto transition-all">
                    ...
                    <div className="space-y-6 flex-1 flex flex-col justify-between">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                        {RATING_CRITERIA && Object.entries(RATING_CRITERIA).map(([catKey, cat]) => (
                          <div key={catKey} className="space-y-2 bg-slate-50/50 p-3 rounded-[20px] border border-slate-100/50">
                            <p className="text-xs cms-min-text-xs font-black text-slate-500 uppercase tracking-widest text-center">{cat.label}</p>
                            <div className="flex flex-wrap justify-center gap-1.5">
                              {cat.options.map(opt => (
                                <button key={opt.key}
                                  onClick={() => setRatingCriteria(prev => ({ ...prev, [catKey]: opt.key }))}
                                  className={`relative px-3 py-1.5 rounded-xl text-xs cms-min-text-xs font-black transition-all outline-none focus:outline-none overflow-hidden flex-1 ${
                      ...
                      <button 
                        ...
                        className={`w-full py-3.5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all relative z-50 cursor-pointer overflow-hidden ${
                        ...
                        <div key={idx} className={`px-4 md:px-6 py-3 md:py-4 flex flex-col md:flex-row md:items-center justify-between ${isCancelled ? 'bg-red-50/30' : 'hover:bg-gray-50'} transition-colors gap-2 md:gap-4`}>
                          ...
                          {!isCancelled && item.grade != null && (
                            <div className="flex items-center gap-2 md:text-right flex-shrink-0 ml-12 md:ml-0">
                              ...
                              <span className={`text-xs cms-min-text-xs md:text-xs font-bold px-2 py-0.5 rounded-full ml-1 ${
