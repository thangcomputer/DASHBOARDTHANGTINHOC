...
export default function CertPrepSingleChoice({
  question,
  value,
  disabled,
  onChange,
  ...
  return (
    <fieldset className="space-y-2.5" disabled={disabled}>
      ...
      {(question.options || []).map((opt, i) => {
        ...
        const isCorrectOpt = showFeedback && i === correct;
        const wrongSelected = showFeedback && isSelected && i !== correct;
        return (
          <label
            key={id}
            htmlFor={id}
            className={`flex items-start gap-3 rounded-xl border px-4 py-3.5 text-sm transition ${
              optionTone({ showFeedback, isCorrectOpt, isSelected, wrongSelected, exam })
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full border text-xs font-black mr-2 align-middle ${
                exam
                  ? (isSelected ? 'bg-emerald-500 border-emerald-400 text-slate-950' : 'border-white/20 text-slate-400')
