...
export default function AiCenterPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (res.success) {
        setResult(res.data);
        if (res.data?.source === 'local' || res.data?.source === 'fallback') {
          toast.success('Bản nháp mẫu (Gemini chưa kết nối — bấm refresh để kiểm tra lại)');
        } else {
          toast.success('AI da tra loi');
        }
      } else {
  ...
  return (
    <div className="cms-viewport-fill w-full">
      ...
      <div className={`rounded-xl border px-3 py-2 text-xs font-bold flex items-start gap-2 shrink-0 ${
        ...
        {status?.healthy ? <CheckCircle2 size={14} className="mt-0.5 shrink-0" /> : <AlertCircle size={14} className="mt-0.5 shrink-0" />}
        <span>
          {status?.healthy
            ? `AI hoạt động · ${status.provider || 'api'} · ${status.model}`
            : status?.configured
              ? `Gemini chưa kết nối — vẫn dùng được bản nháp gợi ý (bấm Chạy AI). ${status.healthError || ''}`
              : 'Chưa có key — thêm GEMINI_API_KEY vào .env rồi restart backend (node server.js)'}
        </span>
      </div>
