...
export default function TeacherScheduleTab({
  ...
  return (
    <div className="px-3 sm:px-4 md:px-0 py-3 sm:py-4 space-y-3 sm:space-y-4 w-full min-w-0 max-w-6xl mx-auto">
      <div className="cms-toolbar sm:flex-row sm:items-center sm:justify-between min-w-0 gap-3">
        <h2 className="text-base sm:text-lg font-bold text-gray-800 flex items-center gap-2 min-w-0">
          <Calendar size={18} className="text-blue-600 shrink-0" /> Lịch dạy
        </h2>
        <button onClick={() => { setEditingSchedule(null); setShowScheduleModal(true); }}
          className="bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold shadow-sm transition flex items-center justify-center gap-1.5 w-full sm:w-auto shrink-0 min-h-9">
